...
class PythonParser(Parser):
    ...
    def file(self) -> Optional[ast.Module]:
        # file: statements? $
        mark = self._mark()
        if (a := self.statements(),) and (_endmarker := self.expect("ENDMARKER")):
    ...
    def global_stmt(self) -> Optional[ast.Global]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (literal := self.expect("global")) and (a := self._gather_18()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Global(
    ...
    def nonlocal_stmt(self) -> Optional[ast.Nonlocal]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (literal := self.expect("nonlocal")) and (a := self._gather_20()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Nonlocal(
                names=[n.string for n in a],
    ...
    def maybe_sequence_pattern(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (patterns := self._gather_70()) and (opt := self.expect(","),):
            return patterns
        self._reset(mark)
        return None
    ...
    def class_pattern(self) -> Optional["ast.MatchClass"]:
        ...
        self._reset(mark)
        if invalid_class_pattern := self.invalid_class_pattern():
            return None  # pragma: no cover
        self._reset(mark)
        return None
    ...
    def star_named_expressions(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_81()) and (opt := self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def slices(self) -> Optional[Any]:
        ...
        if (a := self.slice()) and self.negative_lookahead(self.expect, ","):
            return a
        self._reset(mark)
        if (a := self._gather_86()) and (opt := self.expect(","),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return (
                ast.Tuple(
    ...
    def double_starred_kvpairs(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_109()) and (opt := self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_targets_list_seq(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_127()) and (opt := self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_target(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (literal := self.expect("*")) and (a := self._tmp_130()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Starred(
    ...
    def del_targets(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_131()) and (opt := self.expect(","),):
            return a
        self._reset(mark)
        return None
        ...
        if (a := self.args()) and (b := self.for_if_clauses()):
            return (
                self.raise_syntax_error_known_range(
                    "Generator expression must be parenthesized", a[0][-1], b[-1].target
                )
                if len(a[0]) > 1
                else None
            )
    ...
    def invalid_expression(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if invalid_legacy_expression := self.invalid_legacy_expression():
            return None  # pragma: no cover
        self._reset(mark)
        if (
            self.negative_lookahead(
    ...
    def invalid_assignment(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            and (literal := self.expect(","))
            and (_loop0_149 := self._loop0_149(),)
            and (literal_1 := self.expect(":"))
            and (expression := self.expression())
        ):
            return self.raise_syntax_error_known_location(
    ...
    def invalid_star_etc(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if (a := self.expect("*")) and (_tmp_160 := self._tmp_160()):
            return self.raise_syntax_error_known_location(
    ...
    def invalid_lambda_star_etc(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if (literal := self.expect("*")) and (_tmp_161 := self._tmp_161()):
    ...
    def invalid_with_stmt(self) -> Optional[None]:
        ...
        if (
            ...
            and (_gather_163 := self._gather_163())
            and (forced := self.expect_forced(self.expect(":"), "':'"))
        ):
            return None  # pragma: no cover
        self._reset(mark)
        if (
            ...
            and (literal := self.expect("with"))
            and (literal_1 := self.expect("("))
            and (_gather_165 := self._gather_165())
    ...
    def invalid_with_stmt_indent(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            and (_gather_167 := self._gather_167())
            and (literal := self.expect(":"))
            and (_newline := self.expect("NEWLINE"))
        ...
        if (
            ...
            and (a := self.expect("with"))
            and (literal := self.expect("("))
            and (_gather_169 := self._gather_169())
            and (opt_1 := self.expect(","),)
            and (literal_1 := self.expect(")"))
            and (literal_2 := self.expect(":"))
            and (_newline := self.expect("NEWLINE"))
    ...
    def invalid_class_argument_pattern(self) -> Optional[list]:
        ...
        if (
            (opt := self._tmp_175(),)
            and (keyword_patterns := self.keyword_patterns())
            and (literal := self.expect(","))
    ...
    def _loop0_116(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (literal := self.expect(",")) and (elem := self._tmp_190()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_115(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_190()) is not None and (
            seq := self._loop0_116()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_160(self) -> Optional[Any]:
        ...
        if literal := self.expect(")"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_195 := self._tmp_195()):
    ...
    def _tmp_161(self) -> Optional[Any]:
        ...
        if literal := self.expect(":"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_196 := self._tmp_196()):
    ...
    def _loop0_164(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (literal := self.expect(",")) and (elem := self._tmp_197()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_163(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_197()) is not None and (
            seq := self._loop0_164()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_166(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (literal := self.expect(",")) and (elem := self._tmp_198()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_165(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_198()) is not None and (
            seq := self._loop0_166()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_168(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (literal := self.expect(",")) and (elem := self._tmp_199()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_167(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_199()) is not None and (
            seq := self._loop0_168()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_170(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (literal := self.expect(",")) and (elem := self._tmp_200()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_169(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_200()) is not None and (
            seq := self._loop0_170()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_190(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (_tmp_201 := self._tmp_201()) and self.negative_lookahead(self.expect, "="):
            return _tmp_201
        self._reset(mark)
        return None
    ...
    def _tmp_197(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_202(),):
    ...
    def _tmp_198(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_203(),):
    ...
    def _tmp_199(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_204(),):
            return [expression, opt]
        self._reset(mark)
        return None
    ...
    def _tmp_200(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_205(),):
            return [expressions, opt]
        self._reset(mark)
        return None
