...
from pegen.parser import memoize, memoize_left_rec, logger, Parser as BaseParser
...
def parse_file(
    path: str,
    py_version: Optional[tuple[int, ...]] = None,
...
class Parser(BaseParser):
    ...
    def parse(self, rule: str, call_invalid_rules: bool = False) -> Any:
...
class PythonParser(Parser):
    ...
    def file(self) -> Optional[ast.Module]:
        # file: statements? $
        mark = self._mark()
        if (a := self.statements(),) and (self.expect("ENDMARKER")):
    ...
    def simple_stmts(self) -> Optional[list]:
        ...
        if (
            ...
            return [a]
        self._reset(mark)
        if (a := self._gather_4()) and (self.expect(";"),) and (self.expect("NEWLINE")):
            return a
        self._reset(mark)
        return None
    ...
    def global_stmt(self) -> Optional[ast.Global]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("global")) and (a := self._gather_18()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Global(
    ...
    def nonlocal_stmt(self) -> Optional[ast.Nonlocal]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("nonlocal")) and (a := self._gather_20()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Nonlocal(
                names=[n.string for n in a],
    ...
    def import_from_as_name(self) -> Optional[ast.alias]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.name()) and (b := self._tmp_28(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.alias(
                name=a.string,
    ...
    def dotted_as_name(self) -> Optional[ast.alias]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.dotted_name()) and (b := self._tmp_31(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.alias(
                name=a,
    ...
    def parameters(self) -> Optional[ast.arguments]:
        ...
        if (
            ...
            and (b := self._loop0_38(),)
            and (c := self._loop0_39(),)
            and (d := self.star_etc(),)
        ):
            return self.check_version(
                (3, 8),
        ...
        if (
            (a := self._loop1_41())
            and (b := self._loop0_42(),)
            and (c := self.star_etc(),)
        ):
            return self.make_arguments(None, [], a, b, c)
        self._reset(mark)
        if (a := self._loop1_43()) and (b := self.star_etc(),):
    ...
    def slash_no_default(self) -> Optional[List[Tuple[ast.arg, None]]]:
        ...
        mark = self._mark()
        if (a := self._loop1_44()) and (self.expect("/")) and (self.expect(",")):
    ...
    def maybe_sequence_pattern(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (patterns := self._gather_70()) and (self.expect(","),):
            return patterns
        self._reset(mark)
        return None
    ...
    def expressions(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.expression()) and (b := self._loop1_79()) and (self.expect(","),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Tuple(
                elts=[a] + b,
    ...
    def star_named_expressions(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_81()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def disjunction(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.conjunction()) and (b := self._loop1_83()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.BoolOp(
                op=ast.Or(),
    ...
    def conjunction(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.inversion()) and (b := self._loop1_84()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.BoolOp(
                op=ast.And(),
    ...
    def comparison(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.bitwise_or()) and (b := self._loop1_85()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Compare(
                left=a,
    ...
    def slices(self) -> Optional[Any]:
        ...
        if (a := self.slice()) and (self.negative_lookahead(self.expect, ",")):
            return a
        self._reset(mark)
        if (a := self._gather_86()) and (self.expect(","),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return (
                ast.Tuple(
    ...
    def atom(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "(")) and (_tmp_89 := self._tmp_89()):
            return _tmp_89
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "[")) and (_tmp_90 := self._tmp_90()):
            return _tmp_90
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "{")) and (_tmp_91 := self._tmp_91()):
            return _tmp_91
        self._reset(mark)
        if self.expect("..."):
            tok = self._tokenizer.get_last_non_whitespace_token()
    ...
    def group(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.expect("(")) and (a := self._tmp_92()) and (self.expect(")")):
            return a
        self._reset(mark)
    ...
    def lambda_parameters(self) -> Optional[ast.arguments]:
        ...
        if (
            ...
            and (b := self._loop0_93(),)
            and (c := self._loop0_94(),)
        ...
        if (
            (a := self._loop1_96())
            and (b := self._loop0_97(),)
            and (c := self.lambda_star_etc(),)
        ):
            return self.make_arguments(None, [], a, b, c)
        self._reset(mark)
        if (a := self._loop1_98()) and (b := self.lambda_star_etc(),):
            return self.make_arguments(None, [], None, a, b)
        self._reset(mark)
    ...
    def lambda_slash_no_default(self) -> Optional[List[Tuple[ast.arg, None]]]:
        ...
        mark = self._mark()
        if (a := self._loop1_99()) and (self.expect("/")) and (self.expect(",")):
            return [(p, None) for p in a]
        self._reset(mark)
        if (
            (a := self._loop1_100())
    ...
    def tuple(self) -> Optional[ast.Tuple]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("(")) and (a := self._tmp_108(),) and (self.expect(")")):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Tuple(
                elts=a or [],
    ...
    def double_starred_kvpairs(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_109()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def args(self) -> Optional[Tuple[list, list]]:
        ...
        mark = self._mark()
        if (a := self._gather_115()) and (b := self._tmp_117(),):
            return (
    ...
    def kwargs(self) -> Optional[list]:
        ...
        if (
            (a := self._gather_118())
            and (self.expect(","))
            and (b := self._gather_120())
        ):
            return a + b
        self._reset(mark)
        if _gather_122 := self._gather_122():
            return _gather_122
        self._reset(mark)
        if _gather_124 := self._gather_124():
            return _gather_124
        self._reset(mark)
        return None
    ...
    def star_targets_list_seq(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_127()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_target(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("*")) and (a := self._tmp_130()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Starred(
    ...
    def del_targets(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_131()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    @memoize
    def invalid_arguments(self) -> NoReturn:
    ...
    @memoize
    def invalid_expression(self) -> NoReturn:
    ...
    @memoize
    def invalid_named_expression(self) -> NoReturn:
    ...
    @memoize
    def invalid_assignment(self) -> NoReturn:
        ...
        self._reset(mark)
        if (self._loop0_152(),) and (a := self.yield_expr()) and (self.expect("=")):
            self.raise_syntax_error_known_location(
                "assignment to yield expression not possible", a
            )
        self._reset(mark)
        if (a := self.star_expressions()) and (self.augassign()) and (self._tmp_153()):
            self.raise_syntax_error_known_location(
    ...
    @memoize
    def invalid_del_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_block(self) -> NoReturn:
    ...
    @memoize
    def invalid_comprehension(self) -> NoReturn:
    ...
    @memoize
    def invalid_dict_comprehension(self) -> NoReturn:
    ...
    @memoize
    def invalid_parameters(self) -> NoReturn:
        ...
        self._reset(mark)
        if (self._tmp_160()) and (self._loop0_161(),) and (a := self.expect("/")):
        ...
        self._reset(mark)
        if (self._loop1_166()) and (self.expect("/")) and (a := self.expect("*")):
    ...
    def invalid_star_etc(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self.expect("*")) and (self._tmp_168()):
            self.raise_syntax_error_known_location(
    ...
    def invalid_parameters_helper(self) -> Optional[Any]:
        ...
        if a := self.slash_with_default():
            return [a]
        self._reset(mark)
        if a := self._loop1_173():
            return a
        self._reset(mark)
        return None

    @memoize
    def invalid_lambda_parameters(self) -> NoReturn:
        ...
        self._reset(mark)
        if (self._tmp_178()) and (self._loop0_179(),) and (a := self.expect("/")):
            self.raise_syntax_error_known_location("/ may appear only once", a)
        self._reset(mark)
        if (
            (self._tmp_180(),)
    ...
    @memoize
    def invalid_double_type_comments(self) -> NoReturn:
    ...
    @memoize
    def invalid_with_item(self) -> NoReturn:
    ...
    @memoize
    def invalid_for_target(self) -> NoReturn:
    ...
    @memoize
    def invalid_group(self) -> NoReturn:
    ...
    @memoize
    def invalid_import_from_targets(self) -> NoReturn:
    ...
    @memoize
    def invalid_with_stmt_indent(self) -> NoReturn:
    ...
    @memoize
    def invalid_try_stmt(self) -> NoReturn:
    ...
    def invalid_except_stmt(self) -> Optional[None]:
        ...
        self._reset(mark)
        if (self.expect("except")) and (self.expect("NEWLINE")):
            self.raise_syntax_error("expected ':'")
        self._reset(mark)
        return None

    @memoize
    def invalid_finally_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_except_stmt_indent(self) -> NoReturn:
    ...
    @memoize
    def invalid_match_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_case_block(self) -> NoReturn:
    ...
    @memoize
    def invalid_as_pattern(self) -> NoReturn:
    ...
    @memoize
    def invalid_class_pattern(self) -> NoReturn:
    ...
    @memoize
    def invalid_if_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_elif_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_else_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_while_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_for_stmt(self) -> NoReturn:
    ...
    @memoize
    def invalid_def_raw(self) -> NoReturn:
    ...
    @memoize
    def invalid_class_def_raw(self) -> NoReturn:
    ...
    @memoize
    def invalid_kvpair(self) -> NoReturn:
    ...
    def _loop0_87(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.slice()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_86(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.slice()) is not None and (seq := self._loop0_87()) is not None:
            return [elem] + seq
        self._reset(mark)
        return None
    ...
    def _loop0_116(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_219()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_115(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_219()) is not None and (
            seq := self._loop0_116()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_168(self) -> Optional[Any]:
        ...
        if literal := self.expect(")"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_224 := self._tmp_224()):
    ...
    def _tmp_186(self) -> Optional[Any]:
        ...
        if literal := self.expect(":"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_225 := self._tmp_225()):
    ...
    def _loop0_193(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_226()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_195(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_227()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_194(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_227()) is not None and (
            seq := self._loop0_195()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_197(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_228()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_196(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_228()) is not None and (
            seq := self._loop0_197()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_199(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_229()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_198(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_229()) is not None and (
            seq := self._loop0_199()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_226(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_231(),):
    ...
    def _tmp_227(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_232(),):
    ...
    def _tmp_228(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_233(),):
            return [expression, opt]
        self._reset(mark)
        return None
    ...
    def _tmp_229(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_234(),):
            return [expressions, opt]
        self._reset(mark)
        return None
