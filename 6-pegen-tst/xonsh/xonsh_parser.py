...
# @generated by pegen from xonsh-full.gram
...
class XonshParser(Parser):
    ...
    def function_def_raw(self) -> Optional[Union [ast . FunctionDef , ast . AsyncFunctionDef]]:
        ...
        if (
            ...
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . AsyncFunctionDef ( name = n . string , args = params or self . make_arguments ( None , [] , None , [] , None ) , returns = a , body = b , type_comment = tc , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , )
        self._reset(mark)
        return None
    ...
    def invalid_arguments(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            return self . raise_syntax_error_known_range ( "Generator expression must be parenthesized" , a [0] [- 1] , b [- 1] . target ) if len ( a [0] ) > 1 else None
        self._reset(mark)
        if (
            (args := self.args())
            and
    ...
    def invalid_named_expression(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            return ( None if self . in_recursive_rule else self . raise_syntax_error_known_range ( f"cannot assign to {self.get_expr_name(a)} here. Maybe you meant '==' instead of '='?" , a , b ) )
        self._reset(mark)
        return None
    ...
    def invalid_kvpair(self) -> Optional[None]:
        ...
        if (
            ...
            return self . raise_raw_syntax_error ( "':' expected after dictionary key" , ( a . lineno , a . col_offset - 1 ) , ( a . end_lineno , a . end_col_offset , - 1 ) )
        self._reset(mark)
        if (
            (expression := self.expression())
            and
    ...
    KEYWORDS = ('raise', 'break', 'assert', 'and', 'import', 'await', 'nonlocal', 'del', 'if', 'else', 'while', 'None', 'async', 'finally', 'lambda', 'or', 'not', 'continue', 'for', 'except', 'False', 'is', 'in', 'class', 'return', 'elif', 'yield', 'def', 'as', 'True', 'pass', 'global', 'from', 'with', 'try')
    SOFT_KEYWORDS = ('_', 'match', 'case')


if __name__ == '__main__':
    from pegen.parser import simple_parser_main
    simple_parser_main(XonshParser)
